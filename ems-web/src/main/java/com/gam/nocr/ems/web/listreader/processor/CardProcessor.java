 ...
 * 
 */
public class CardProcessor extends EMSVLPListProcessor {

	private static final Logger logger = BaseLog.getLogger(CardProcessor.class);
	...
	public ListResult fetchList(ParameterProvider paramProvider)
		...
		try {
			...
			List<CardDispatchInfoVTO> page = null;
			...
			{
				page =new ArrayList<CardDispatchInfoVTO>();
			}
			Integer count = page.size();
			if (criteria.getPageSize() == count) {
				count = new CardRequestDelegator().countCardLostTemp(criteria);
			} else if (criteria.getPageNo() > 0) {
				count += criteria.getPageNo() * criteria.getPageSize();
			}
			logger.info("fetch lost card. list name is: "
	...
	private GeneralCriteria getCriteria(ParameterProvider paramProvider)
		...
		try {
			...
			throw new ListReaderException(
					"Unable to load user department in order to limit the card lost list result to his department scope",
					e);
		}
