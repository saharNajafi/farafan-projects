    ...
    private void verifyExtendIssuance(List<CardInfoVTO> allCardsList) throws BaseException {
        try {
            if (allCardsList != null
                ...
                String toleranceValid =
                        EmsUtil.getProfileValue(
                                ProfileKeyName.KEY_DURATION_OF_VALID_YEAR_TOLERANCE_FOR_EXTEND, null);
                ...
                if (issuanceDate != null) {
                    ...
                    if (toleranceValid != null) {
                        Integer toleranceValue = 0;
                        if (toleranceValid.contains("y")) {
                            toleranceValue = Integer.valueOf(toleranceValid.split("y")[0]);
                            validDate = EmsUtil.getDateAtMidnight(EmsUtil.differYear(validDate, -toleranceValue));
                        } else if (toleranceValid.contains("M")) {
                            toleranceValue = Integer.valueOf(toleranceValid.split("M")[0]);
                            validDate = EmsUtil.getDateAtMidnight(EmsUtil.differMonth(validDate, -toleranceValue));
                        } else if (toleranceValid.contains("d")) {
                            toleranceValue = Integer.valueOf(toleranceValid.split("d")[0]);
                            validDate = EmsUtil.getDateAtMidnight(EmsUtil.differDay(validDate, -toleranceValue));
                        }
                    }
